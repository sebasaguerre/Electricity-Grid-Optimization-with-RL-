...
price_space = spaces.Discrete(len(price_ranges))
action_space = spaces.Discrete(3)
storage_values = np.arange(0, 201, 10)
storage_space = spaces.Discrete(len(storage_values))
...
Q_table = torch.zeros((storage_space.n, price_space.n, action_space.n), device=device)
nnjhuo
...
for i in range(num_episodes):
    if i < 10:
        ...
        epsilon = 0.02
    terminated = False
    state = environment.reset() # [storage_level, price, hour, day]
    print("Episode:", i)
    # print("State:", state)
