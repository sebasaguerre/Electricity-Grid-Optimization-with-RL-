...
def get_state_indices(storage, price, hour):
...
parser.add_argument('--save_q', type=str, default='small_qtable_noday_2k_new.pth', help='Filename to save learned Q-table')
...
remark_save_intermediate_qtable = "small_noday_2k_new"
reward_file_name = "training_rewards_smallq_noday_2k_new.npy"
val_reward_file_name = "validation_rewards_smallq_noday_2k_new.npy"    
...
def train_qlearn(alpha, gamma, episodes, device, start_epsilon, end_epsilon):
    ...
    # Q_shape = (22, 6, 5, 3) # storage, price, hour, action [maybe reduce the storage bins?]
    ...
    for ep in range(episodes):
        ...
        s_idx, p_idx, h_idx = get_state_indices(state[0], state[1], state[2])
        ...
        while not done:
            ...
            s2_idx, p2_idx, h2_idx = get_state_indices(next_state[0], next_state[1], next_state[2])

            # Q-learning update
            # best_next = torch.max(Q[s2_idx, p2_idx, h2_idx, d2_idx]).item()
            best_next = torch.max(Q[s2_idx, p2_idx, h2_idx]).item()
            # current_q = Q[s_idx, p_idx, h_idx, d_idx, a_idx].item()
            current_q = Q[s_idx, p_idx, h_idx, a_idx].item()
            # Q[s_idx, p_idx, h_idx, d_idx, a_idx] = torch.tensor(
            #     current_q + alpha * (reward + gamma * best_next - current_q),
            #     device=device
            # )
            Q[s_idx, p_idx, h_idx, a_idx] = torch.tensor(
                current_q + alpha * (reward + gamma * best_next - current_q),
                device=device
            )
            ...
            s_idx, p_idx, h_idx = s2_idx, p2_idx, h2_idx
...
def validate_qlearn(Q, env_path):
    ...
    while not done:
        ...
        s_idx, p_idx, h_idx = get_state_indices(next_state[0], next_state[1], next_state[2])
...
def visualize_rewards(rewards):
    import matplotlib.pyplot as plt
    for rwd_list in rewards:
        plt.plot(rwd_list)
    # plt.plot(rewards)
    plt.title("Training Rewards")
    plt.legend(["Reward scaling", "Risk factor + rwd scaling"])
    plt.xlabel("Episodes")
...
def main():
    ...
    if validate:
        path_to_qtable = "small_qtable.pth"
        ...
        np.save(val_reward_file_name, np.array(val_reward))
    ...
    if training:
        visualize_rewards(train_rewards)
    else:
        # load_train_rewards1 = np.load('training_rewards_smallq_noday.npy')
        load_train_rewards2 = np.load('training_rewards_smallq_noday_risk.npy')
        load_train_rewards3 = np.load('training_rewards_smallq_noday_2k.npy')
        # load_train_rewards4 = np.load('training_rewards_smallq_noday_2k_new.npy')
        train_rwd_list = [load_train_rewards2, load_train_rewards3]
        # val_rwd_list = [np.load('validation_rewards_smallq_noday.npy'), np.load('validation_rewards_smallq_noday_risk.npy')]
        # visualize_rewards(val_rwd_list[0])
        visualize_rewards(train_rwd_list)
